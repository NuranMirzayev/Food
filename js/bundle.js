...
function slides(_ref) {
  ...
  if (slides.length < 10) {
    ...
    current.textContent = "0".concat(indexSlider);
  } else {
  ...
  for (var i = 0; i < slides.length; i++) {
    var dot = document.createElement('il');
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClassSelector) {
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //slice etim sposobom mi udalayem tocku(pervuyu simval) tk tabsSelector prinimayet argument v takom vvide '.tabheader__item'
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var today = new Date();
  var deadline = new Date(today.setDate(today.getDate() + 20));
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_9__["default"])('.timer', deadline);
